...
distance = np.ones((100,100))*np.inf
distance[y,x] = 0
commandchain = {50*100+50 : []}
d0 = -1
...
while c.ACTIVE:
    ...
    if o == 1:
        if distance[y,x]+1 < distance[y+dy(d),x+dx(d)]:
            distance[y+dy(d),x+dx(d)] = distance[y,x]+1
            # commandchain.update({[x+dx(d),y+dy(d)] : commandchain[[x,y]].append(d)})
        ...
        field[y+dy(d),x+dx(d)] = 2

    # ind = np.where(field==1)
    # indx = ind[1]
    # indy = ind[0]
    # if np.all(field[ind+1] == 0

revfield = field[::-1,:]
plt.pcolor(revfield[23:75,23:75])

gral = np.where(field==3)
gralpos = np.vstack(gral)
print(distance[gralpos[0],gralpos[1]])

distance_from_oxygen = np.ones((100,100))*np.inf
distance_from_oxygen[gralpos[0],gralpos[1]] = 0
...
while c.ACTIVE:
    ...
    if o == 1:
        if distance_from_oxygen[y,x]+1 < distance_from_oxygen[y+dy(d),x+dx(d)]:
            distance_from_oxygen[y+dy(d),x+dx(d)] = distance_from_oxygen[y,x]+1
            # commandchain.update({[x+dx(d),y+dy(d)] : commandchain[[x,y]].append(d)})
    ...
    if np.sum(distance_from_oxygen[np.where(field==1)]) < np.inf:
        break

print(np.amax(distance_from_oxygen[distance_from_oxygen<np.inf]))