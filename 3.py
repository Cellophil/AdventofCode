...
for s in Line2.split(','):
    ...
    if s[0] == 'R':
        ...
        devy += d
    mdevx = max(mdevx,abs(devx))
    mdevy = max(mdevy,abs(devy))

yoff = -17000
xoff = -14000
playground = np.ones((28000,34000,2),dtype=np.int32)
playground *= np.int32(1000000)
...
for index,Line in enumerate([Line1,Line2]):
    ...
    for s in Line.split(','):
        d = int(s[1:])
        if s[0] == 'R':
            playground[x:x+d+1,y,index] = np.minimum(playground[x:x+d+1,y,index],L+np.arange(0,d+1))
            x += d
        elif s[0] == 'L':
            playground[x:x-d-1:-1,y,index] = np.minimum(playground[x:x-d-1:-1,y,index],L+np.arange(0,d+1))
            x -= d
        elif s[0] == 'D':
            playground[x,y:y-d-1:-1,index] = np.minimum(playground[x,y:y-d-1:-1,index],L+np.arange(0,d+1))
            y -= d
        elif s[0] == 'U':
            playground[x,y:y+d+1,index] = np.minimum(playground[x,y:y+d+1,index],L+np.arange(0,d+1))
            y += d
        L += d



cross = np.where(np.all(playground[:,:] < 1000000,axis=2))
cross = np.vstack(cross).T
dist = np.zeros(len(cross))
for i in range(len(cross)):
    x = cross[i,0]
    y = cross[i,1]
    dist[i] = playground[x,y,0] + playground[x,y,1]
dist.sort()
print(dist[0:10])